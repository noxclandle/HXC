  ...
  return (
    <section className="p-4 md:p-8 border border-white/5 bg-white/[0.01] relative overflow-hidden group">
       <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-10 border-b border-white/5 pb-6 sm:pb-4 gap-4">
          <h2 className="text-[9px] md:text-[10px] tracking-[0.3em] md:tracking-[0.5em] uppercase opacity-30 font-bold italic">プロフィール情報</h2>
          <div className="flex items-center justify-between w-full sm:w-auto gap-4 md:gap-6">
            ...
            <div className="flex gap-1 md:gap-2 p-1 bg-white/5 border border-white/5">
               <button onClick={() => updateOrientation('horizontal')} className={`p-1.5 transition-all ${localOrientation === 'horizontal' ? 'bg-azure-600 text-white' : 'opacity-20 hover:opacity-100'}`} title="横向き">
                  <Layout size={12}/>
               </button>
               <button onClick={() => updateOrientation('vertical')} className={`p-1.5 transition-all ${localOrientation === 'vertical' ? 'bg-azure-600 text-white' : 'opacity-20 hover:opacity-100'}`} title="縦向き">
                  <Smartphone size={12}/>
               </button>
            </div>
            <Link href={`/p/${user.slug}`} className="text-[8px] uppercase tracking-[0.2em] md:tracking-[0.4em] opacity-20 hover:opacity-100 transition-opacity flex items-center gap-2 border-l border-white/10 pl-4 md:pl-6">公開ページを確認 <Share2 size={10}/></Link>
          </div>
       </div>
       ...
       <div className="grid grid-cols-2 gap-4 mt-12">
          <div className="flex flex-col gap-4">
             <Link href="/profile/edit" className="flex flex-col items-center justify-center p-6 bg-white/[0.02] border border-white/5 hover:bg-white/5 transition-all group/btn">
                ...
                <span className="text-[9px] tracking-[0.4em] uppercase font-bold text-white">プロフィール編集</span>
             </Link>
          </div>
          <div className="flex flex-col gap-4">
             <Link href="/inventory" className="flex flex-col items-center justify-center p-6 bg-white/[0.02] border border-white/5 hover:bg-white/5 transition-all group/btn">
                ...
                <span className="text-[9px] tracking-[0.4em] uppercase font-bold text-white">ショップ・装備変更</span>
             </Link>
          </div>
       </div>
    </section>
