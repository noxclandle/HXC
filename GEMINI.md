...
*   **Layout Preservation (Mobile Viewports):** To prevent breaking the mobile user experience, the compact rendering scales of `UnifiedCardContainer` (e.g., `scale-[0.5]` / `scale-[0.4]`) and the relative (non-sticky) document flow of the preview container on mobile in the profile edit screen must be preserved. Never introduce full-screen sticky preview layers that block the active input fields and keyboard on mobile devices.
