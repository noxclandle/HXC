...
export default function PublicProfilePage({ params }: { params: { slug: string } }) {
  ...
  return (
    <main className="min-h-screen bg-void text-moonlight relative overflow-x-hidden overflow-y-auto custom-scrollbar flex flex-col items-center">
      ...
      <motion.section initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} className="relative z-10 w-full max-w-5xl py-48 px-6 border-t border-white/5 space-y-32">
         <div className="text-center space-y-12">
            ...
            <p className="text-[12px] tracking-[0.3em] leading-loose opacity-40 uppercase max-w-xl mx-auto italic text-white">Hexa Cardは、物理的な手触りとデジタルの無限性を融合させた、<br className="hidden md:block" />最高級のアイデンティティ管理プラットフォームです。</p>
         </div>
         ...
         <div className="grid grid-cols-1 md:grid-cols-3 gap-12 px-6">
            {[
              ...
              <div key={i} className="space-y-4 p-8 border border-white/5 bg-white/[0.01] hover:bg-white/[0.03] transition-all group">
                 ...
                 <p className="text-[9px] tracking-widest leading-loose opacity-40 uppercase text-white">{f.desc}</p>
              </div>
            ))}
         </div>
