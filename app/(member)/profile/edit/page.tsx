  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      reader.onloadend = () => {
        ...
        showToast("Photo Selected / 画像を仮設定しました", "info");
      };
           ...
           <form onSubmit={handleSave} className="space-y-12">
              <div className="space-y-6">
                 ...
                 <div className="flex items-center gap-8">
                    ...
                    <button type="button" onClick={() => fileInputRef.current?.click()} className="px-6 py-3 border border-white/10 bg-white/[0.02] text-[9px] tracking-[0.3em] uppercase hover:border-azure-400 transition-all flex items-center gap-3">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                 <div className="space-y-3">
                    ...
                    <input type="text" value={formData.name} onChange={(e) => setFormData({...formData, name: e.target.value})} className="w-full bg-white/[0.03] border border-white/10 p-4 text-sm tracking-widest focus:border-azure-400 outline-none" placeholder="漢字・英語" />
                 </div>
                 <div className="space-y-3">
                    <label className="text-[9px] tracking-[0.4em] uppercase opacity-40 font-bold">Reading / ふりがな</label>
                    <input type="text" value={formData.reading} onChange={(e) => setFormData({...formData, reading: e.target.value})} className="w-full bg-white/[0.03] border border-white/10 p-4 text-sm tracking-widest focus:border-azure-400 outline-none" placeholder="ひらがな" />
                 </div>
                 <div className="space-y-3">
                    <label className="text-[9px] tracking-[0.4em] uppercase opacity-40 font-bold">Company / 所属企業</label>
                    <input type="text" value={formData.company} onChange={(e) => setFormData({...formData, company: e.target.value})} className="w-full bg-white/[0.03] border border-white/10 p-4 text-sm tracking-widest focus:border-bronze-400 outline-none" placeholder="企業名" />
                 </div>
                 <div className="space-y-3">
                    <label className="text-[9px] tracking-[0.4em] uppercase opacity-40 font-bold">Website / ウェブサイト</label>
                    <input type="url" value={formData.website} onChange={(e) => setFormData({...formData, website: e.target.value})} className="w-full bg-white/[0.03] border border-white/10 p-4 text-sm focus:border-azure-400 outline-none" placeholder="https://..." />
                 </div>
              </div>

              <div className="space-y-3">
                 <label className="text-[9px] tracking-[0.4em] uppercase opacity-40 font-bold">Professional Bio / 自己紹介</label>
                 <textarea rows={4} value={formData.bio} onChange={(e) => setFormData({...formData, bio: e.target.value})} className="w-full bg-white/[0.03] border border-white/10 p-4 text-sm focus:border-azure-400 outline-none resize-none" placeholder="実績や専門分野" />
              </div>
              ...
              <div className="pt-8">
                 <button type="submit" disabled={isSaving} className={`w-full py-6 bg-azure-600 text-white font-bold text-[11px] tracking-[1.2em] uppercase shadow-2xl hover:bg-azure-500 transition-all active:scale-[0.98] flex items-center justify-center gap-4 relative overflow-hidden ${isSaving && 'opacity-50'}`}>
                    {isSaving ? "Synchronizing..." : <><Save size={14} /> Commit Changes / 情報を記録</>}
