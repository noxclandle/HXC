...
function RegisterContent() {
  ...
  useEffect(() => {
    if (uid) {
      // 注文情報を取得して、申し込み時の名前を初期値としてセットする
      fetch(`/api/admin/card/list`) 
        .then(res => res.json())
        .then(cards => {
          const card = cards.find((c: any) => c.uid === uid);
          if (card && card.purchaseName !== "-") {
            setFormData(prev => ({ 
              ...prev, 
              name: prev.name || card.purchaseName, 
              purchase_name: card.purchaseName 
            }));
          }
        })
        .catch(() => {});
